...
export default function Tests() {
  ...
  const [suggestedPatients, setSuggestedPatients] = useState(null);
  const [suggestedFile, setSuggestedFile] = useState(null);
  const [isNewPatientCollision, setIsNewPatientCollision] = useState(false);

  const submitWithPatientId = async (patientId, file) => {
    setSuggestedPatients(null);
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.uploadPatientReport(patientId), {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${token}`
        },
      ...
      if (result.success) {
        setNewTest({ email: "", phone: "", file: null });
        setShowAddModal(false);
        fetchTests();
        setNotification({
          type: "success",
          title: "Test Added Successfully!",
          message: "The diagnostic test has been registered and attached to the existing patient.",
          details: [{ label: "Report ID", value: `#${result.report_id || "N/A"}` }],
          actions: [{ label: "Close", handler: () => setNotification(null), style: "bg-blue-600 hover:bg-blue-700" }],
        });
      } else {
        setNotification({
          type: "error",
          title: "Upload Failed",
          message: result.message || "Failed to upload the report.",
          actions: [{ label: "Close", handler: () => setNotification(null), style: "bg-red-600 hover:bg-red-700" }],
        });
      }
    } catch (error) {
      console.error("Error uploading report:", error);
      setNotification({
        type: "error",
        title: "Upload Error",
        message: "Unable to upload the report.",
        actions: [{ label: "Close", handler: () => setNotification(null), style: "bg-red-600 hover:bg-red-700" }],
      });
    } finally {
  ...
  const handleAddTest = async (e, forceCreate = false) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setIsNewPatientCollision(false);
        // Refresh the tests list
        fetchTests();
        ...
        if (result.suggestedPatients && result.suggestedPatients.length > 0) {
          setSuggestedPatients(result.suggestedPatients);
          setSuggestedFile(newTest.file);
          setIsNewPatientCollision(result.isNewPatient === true);
        } else {
          setNotification({
            type: "error",
            title: "Failed to Add Test",
            message:
              "RAW RESPONSE: " + JSON.stringify(result) + " | " + (result.message || "An unexpected error occurred while adding the test."),
            actions: [
              {
                label: "Try Again",
                handler: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add Test Modal Overlay (hide when suggestions open) */}
      {showAddModal && suggestedPatients && (
          ...
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full relative">
              <div className="absolute top-0 right-0 pt-4 pr-4">
                <button
                  onClick={() => setSuggestedPatients(null)}
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        The email or phone number provided belongs to a different patient, but we found the following patient(s) in the database with matching name and details. Would you like to attach the report to one of them?
                      </p>
                    </div>
                  </div>
                </div>
                <div className="mt-5 space-y-3">
                  {suggestedPatients.map(p => (
                    <div key={p.id} className="border rounded-lg p-4 bg-gray-50 flex justify-between items-center">
                      <div>
                        <p className="font-medium text-gray-900">{p.name} <span className="text-xs text-gray-500">(ID: {p.id})</span></p>
                        <p className="text-sm text-gray-600 mt-1">{p.email} {p.phone && `| ${p.phone}`}</p>
                        <p className="text-xs text-gray-500 mt-1">Age: {p.age} | Gender: {p.gender}</p>
                      </div>
                      <button
                        onClick={() => submitWithPatientId(p.id, suggestedFile)}
                        className="ml-4 bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
